...
rule tophat2_align:
    ...
    output:
        "results/rnaseq/tophat2/{datasets}/{rep}/{trimmer}/{strand}/aligned.bam"
    params:
        bowtie2_index = "data/indexes/bowtie2/bowtie2",
        out_dir = "results/rnaseq/tophat2/{datasets}/{rep}/{trimmer}/{strand}",
        strand = lambda w: config["options"][w.strand]["tophat2"]
    conda:
        "../envs/tophat2.yaml"
    threads:
        32
    log:
        "logs/tophat2/align_{datasets}_{rep}_{trimmer}_{strand}.log"
    shell:
        "tophat "
        "--num-threads {threads} "
...
rule HISAT2_align:
    ...
    output:
        "results/rnaseq/HISAT2/{datasets}/{rep}/{trimmer}/{strand}/aligned.bam"
    params:
        out_dir = "results/rnaseq/HISAT2/{datasets}/{rep}/{trimmer}/{strand}",
        strand = lambda w: config["options"][w.strand]["hisat2"]
    conda:
        "../envs/HISAT2.yaml"
    threads:
        32
    log:
        "logs/HISAT2/align_{datasets}_{rep}_{trimmer}_{strand}.log"
    shell:
        "mkdir -p {params.out_dir} && "
        "(hisat2 "
        "--threads {threads} {params.strand} "
