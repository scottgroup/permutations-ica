...
def get_sorted_bam(wildcards):
    ...
    if wildcards["aligner"] == 'STAR':
        ...
        st = "results/rnaseq/{aligner}/{datasets}/{rep}/{trimmer}/{strand}/aligned.sorted.bam"
...
def get_all_sorted_bam(wildcards):
    ...
    for tissue, datasets in config['datasets'].items():
        ...
        wildcard["strand"] = wildcards["strand"]
...
rule cufflinks:
    ...
    input:
        ...
        bam = get_sorted_bam
    output:
        "results/rnaseq/cufflinks/{datasets}/{rep}/{trimmer}/{strand}/{aligner}/{annotation}/counts.tsv"
    params:
        out_dir = "results/rnaseq/cufflinks/{datasets}/{rep}/{trimmer}/{strand}/{aligner}/{annotation}",
        strand = lambda w: config["options"][w.strand]["cufflinks"]
    conda:
        "../envs/cufflinks.yaml"
    threads:
        32
    log:
        "logs/cufflinks/{datasets}_{rep}_{trimmer}_{aligner}_{strand}_{annotation}.log"
    shell:
        "cuffquant "
        "--num-threads {threads} "
...
rule htseq:
    ...
    input:
        ...
        bam = get_sorted_bam
    output:
        "results/rnaseq/htseq/{datasets}/{rep}/{trimmer}/{strand}/{aligner}/{annotation}/counts.tsv"
    params:
        out_dir = "results/rnaseq/htseq/{datasets}/{rep}/{trimmer}/{strand}/{aligner}/{annotation}",
        strand = lambda w: config["options"][w.strand]["htseq"]
    conda:
        "../envs/htseq.yaml"
    threads:
        32
    log:
        "logs/htseq/{datasets}_{rep}_{trimmer}_{aligner}_{strand}_{annotation}.log"
    shell:
        "htseq-count "
        "--format bam "
...
rule featureCounts:
    ...
    input:
        ...
        bam = get_sorted_bam
    output:
        "results/rnaseq/featureCounts/{datasets}/{rep}/{trimmer}/{strand}/{aligner}/{annotation}/counts.tsv"
    params:
        strand = lambda w: config["options"][w.strand]["featureCounts"]
    conda:
        "../envs/subread.yaml"
    threads:
        32
    log:
        "logs/featureCounts/{datasets}_{rep}_{trimmer}_{aligner}_{strand}_{annotation}.log"
    shell:
        "featureCounts "
        "-T {threads} "
        "-a {input.gtf} "
